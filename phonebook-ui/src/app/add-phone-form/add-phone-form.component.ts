...
export class AddPhoneFormComponent implements OnInit, OnDestroy {
    ...
    onSubmit(): void {
        ...
        const addSubscription = this.phoneService.addPhone(this.phoneForm.value)
            .subscribe(value => this.addPhone.emit(value),
                error => this.addPhoneError(error));
    ...
    private addPhoneError(error: HttpErrorResponse): void {
        if (error.status === 500)
            this.logError = 'Server error 500, please try again later';
        else if (error.error.errorMsg)
            this.logError = error.error.errorMsg;
        else
            this.logError = 'Server error, please try again later';
